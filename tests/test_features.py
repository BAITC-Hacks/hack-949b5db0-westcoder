...
class FeatureTests(unittest.TestCase):
    ...
    def test_embedding_failure_explicit_retry_and_cache_keys_include_candidates(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(matcher,'_embed',side_effect=TimeoutError):
                self.assertEqual(matcher.similarities(event,candidates,'fp')[1],'unavailable')
            with patch.object(matcher,'_embed',side_effect=lambda texts:[[1.,0.] for _ in texts]) as embed:
                self.assertEqual(matcher.similarities(event,candidates,'fp')[1],'unavailable')
                embed.assert_not_called()
                self.assertEqual(matcher.similarities(event,candidates,'fp',refresh=True)[1],'embeddings')
